---
title: "HW 03 - Money in US politics"
output: 
  tufte::tufte_html:
    css: hw.css
...
``` {r name-separation, eval = TRUE}
pac_all <- pac_all %>%
  separate(country_parent, c("country", "parent"), sep = "/", extra = "merge")
...
pac_all <- pac_all %>%
  mutate(
    total = clean_dollars(total),
    dems = clean_dollars(dems),
    repubs = clean_dollars(repubs)
  )
...
pac_all %>%
  ...
  labs(
    x = "Year",
    y = "$ Amount",
    title = "Contribution to US politics from Mexico and Canada-Connected PACs"
  )

```

The visualization reveals that Mexico and Canada, while initially having a similar rate of increase in foreign-connected PACs for the US from the 2000's to around 2008, Canada had an increasing amount donated to the United States, peaking in 2019 before slightly decreasing. For this 20-year window, Mexico has had lower donations compared to that of Canada, and has seen decreasing donations since 2016. In fact, there were a few years in the early 2000's in which Mexico contributed no donations to US affiliates.
...
```{r eval = TRUE, fig.fullwidth = TRUE, fig.asp = 0.5}
pac_all2 <- read_csv("data/pac-all.csv")
...
pac_all2 %>%
  ...
  theme_minimal()
```
The graph reveals that there is an increasing amount of republican support from the UK regarding PACs, although recent trends show republican donations decreasing and democratic donations plateauing. In the past 20-year window, it can reasonably be inferred that republicans have had a stronger backing from the UK, although the future trajectory is uncertain. 2010 was the strongest year for democratic support from the UK.
